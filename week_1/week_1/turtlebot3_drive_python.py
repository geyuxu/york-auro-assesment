            ...
            case State.GET_TB3_DIRECTION:
                ...
                if self.scan_data_[CENTER] > check_forward_dist:
                    if self.scan_data_[LEFT] < check_side_dist:
                        ...
                        self.prev_state = self.turtlebot3_state_num
                        ...
                        if now - self.time >= Duration(seconds=2.0) and self.first_time:
                            # Stop once we have been going straight for 2 seconds
                            self.get_logger().info("GET_TB3_DIRECTION {stopping}")
                            self.update_cmd_vel(0.0, 0.0)
                            self.first_time = False
                        elif self.first_time:
                            if (self.prev_state != self.turtlebot3_state_num):
                                self.get_logger().info("GET_TB3_DIRECTION {moving forward}")
                                self.prev_state = self.turtlebot3_state_num
